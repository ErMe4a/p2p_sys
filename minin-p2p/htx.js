...
function collectFormData() {
    ...
    if (formData.hasReceipt) {
        // ВАЖНО: Эвотор требует не более 3 знаков после запятой для количества
        let rawAmount = parseNumberOrNull(quantityInput ? quantityInput.value : null);
        let validAmount = rawAmount;
        if (rawAmount !== null) {
            // Округляем до 3 знаков, чтобы не злить Эвотор
            validAmount = Math.floor(rawAmount * 1000) / 1000;
        }

        formData.receipt = {
            contact: contactInput ? contactInput.value : '',
            price: parseNumberOrNull(rateInput ? rateInput.value : null),
            amount: validAmount, // Исправленное количество для чека
    ...
    // Для внутренней статистики сохраняем полную точность
