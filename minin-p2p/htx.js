...
function parseOrderInfo() {
    ...
    // Основной формат даты на HTX: 2024-01-30 15:30:00
    // Иногда бывает: 2024/01/30 или просто время, если сегодня
    const dateRegex = /(\d{4}[-/]\d{2}[-/]\d{2}\s+\d{2}:\d{2}:\d{2})/;

    try {
        console.log('P2P Analytics HTX: Searching for creation date...');
        
        // Способ 1: Ищем дату в ключевых блоках (Детали ордера)
        // .l-trade-detail - основной блок информации
        // .baseInfo - иногда используется
        ...
        for (const block of contentBlocks) {
            ...
            if (match) {
                // Нашли дату!
                // Меняем слеши на тире для совместимости (2024/01/01 -> 2024-01-01)
                const dateStr = match[1].replace(/\//g, '-');
                const parsedDate = new Date(dateStr);
                
                if (!isNaN(parsedDate.getTime())) {
                    orderInfo.createdAt = parsedDate.toISOString();
                    console.log('P2P Analytics HTX: ✓ Found date in block:', dateStr);
                    dateFound = true;
                    break;
                }
            }
        ...
        // Способ 2 (Если в блоках не нашли): Ищем по всему тексту тела страницы (Brute Force)
        // Это гарантирует нахождение, если дата вообще есть на экране
        if (!dateFound) {
            const bodyText = document.body.innerText; // innerText берет только видимый текст
    ...
    // FALLBACK: Если совсем ничего не нашли, берем текущее время
    if (!dateFound) {
        console.warn('P2P Analytics HTX: ⚠ Date still not found, using CURRENT time.');
