...
// Вспомогательные функции для определения цвета (RGB)
function isColorRed(colorStr) {
    if (!colorStr) return false;
    // #FF4761 = rgb(255, 71, 97)
    if (colorStr.includes('255, 71, 97') || colorStr.includes('255, 71, 97')) return true;
    
    // Общая проверка на красный оттенок
    const rgb = colorStr.match(/\d+/g);
    if (rgb && rgb.length >= 3) {
        const [r, g, b] = rgb.map(Number);
        return r > 200 && g < 100 && b < 120;
    }
    return false;
}

function isColorGreen(colorStr) {
    if (!colorStr) return false;
    // #0BBA74 = rgb(11, 186, 116) или #0B8E5A
    if (colorStr.includes('11, 186, 116') || colorStr.includes('11, 142, 90')) return true;

    // Общая проверка на зеленый оттенок
    const rgb = colorStr.match(/\d+/g);
    if (rgb && rgb.length >= 3) {
        const [r, g, b] = rgb.map(Number);
        return g > 140 && r < 100; 
...
function detectOrderType() {
    // 1. ЖЕЛЕЗНЫЙ ВАРИАНТ: Проверка по конкретным классам MEXC
    
    // Ищем элемент суммы (он есть и там, и там)
    const amountEl = document.querySelector('.page_orderAmount__rm9xj');

    if (amountEl) {
        // Проверяем наличие класса ПРОДАЖИ
        const isSellClass = amountEl.classList.contains('page_orderSellAmount__MKos9');
        
        // Проверяем цвет
        const style = window.getComputedStyle(amountEl);
        const color = style.color;
        
        // ЛОГИКА ПРОДАЖИ (ПРИХОД)
        if (isSellClass || isColorRed(color)) {
            console.log('P2P Analytics MEXC: Detected SELL (Income) via class/color');
            return 'sell';
        }

        // ЛОГИКА ПОКУПКИ (РАСХОД)
        // Если это не продажа, но есть базовый класс amountEl - значит это покупка.
        // Дополнительно проверяем зеленый цвет для уверенности.
        if (!isSellClass || isColorGreen(color)) {
            console.log('P2P Analytics MEXC: Detected BUY (Expense) via class/color');
            return 'buy';
        }
    }

    // 2. ЗАПАСНОЙ ВАРИАНТ (Если классы поменяются)
    // Проверка заголовков и текста
    const textContext = document.body.innerText.substring(0, 2000).toLowerCase();
    
    // Красные ключевые слова (Продажа)
    if (textContext.includes('продать usdt') || textContext.includes('sell usdt')) return 'sell';
    
    // Зеленые ключевые слова (Покупка)
    if (textContext.includes('купить usdt') || textContext.includes('buy usdt')) return 'buy';

    // По умолчанию, если ничего не понятно, возвращаем unknown, 
    // но collectFormData поставит BUY
    return 'unknown';
}
...
const uploadScreenshotFromDataUrl = async (dataUrl, orderId) => {
    try {
        ...
        // ИСПРАВЛЕНИЕ: Убрали префикс 'mexc_', так как сервер ищет ордер строго по ID
        // Было: `mexc_${orderId}.png` -> Стало: `${orderId}.png`
...
// Wrapper for checkOrderExists with MEXC exchange type
// Wrapper for checkOrderExists with MEXC exchange type
checkOrderExists = async (orderId) => {
    try {
        ...
        // ИЗМЕНЕНИЕ: Используем основной эндпоинт /api/order, так как ТОЛЬКО ОН возвращает поле 'receipt'
        // В views.py метод GET order принимает 'id' как external_id
        const url = `${window.P2PAuth.API_BASE_URL}/api/order?id=${encodeURIComponent(orderId)}&exchangeType=${EXCHANGE_TYPE_MEXC}`;
        
        console.log(`P2P Analytics MEXC: Checking order details at ${url}`);
        ...
        try {
            ...
            console.error('P2P Analytics MEXC: Server returned non-JSON:', text);
        ...
        if (response.ok) {
            ...
            return {
                success: false,
                error: (data && data.message) ? data.message : `HTTP ${response.status}`
            };
        }
    } catch (error) {
...
// Custom saveOrder for MEXC
// Custom saveOrder for MEXC with RECEIPT DEBUGGING
saveOrder = async (orderData) => {
    try {
        ...
        console.log('P2P Analytics MEXC: Sending payload:', JSON.stringify(orderData));
        ...
        if (response.ok && data) {
            ...
            if (orderData.hasReceipt && data.receipt) {
                console.log("P2P Analytics MEXC: Receipt Server Response:", data.receipt);
                
                // Если сервер вернул ошибку по чеку, ПОКАЖЕМ ЕЁ
                if (data.receipt.status === 'ERROR' || (data.receipt.error && data.receipt.error.length > 0)) {
                    console.error("P2P Analytics MEXC: Receipt Error:", data.receipt.error);
                    // Выводим алерт, чтобы ты сразу увидел причину
                    alert(`Ордер сохранен, но ЧЕК НЕ ПРОБИТ!\nОшибка Эвотора: ${data.receipt.error}`);
                } else if (data.receipt.status === 'SENT') {
                    console.log("P2P Analytics MEXC: Receipt sent successfully!");
                }
            }
            // ========================
            ...
            return {
                success: true,
                orderId: data.id || responseText, // API может вернуть JSON {id: 123} или просто текст
                message: 'Order saved'
            };
        } else {
             ...
             console.error('P2P Analytics MEXC: Save failed:', errorMsg);
             return { success: false, error: errorMsg };
        }
    } catch (error) {
        console.error('P2P Analytics MEXC: Network error:', error);
        return { success: false, error: error.message };
    }
};
