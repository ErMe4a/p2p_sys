...
function collectFormData() {
    ...
    formData.commission = commissionInput ? (cleanAndParseFloat(commissionInput.value) || 0) : 0;
    ...
    // --- СБОР ОСНОВНЫХ ДАННЫХ (ВСЕГДА) ---
    const contactInput = document.querySelector('#contact-input');
    const rateInput = document.querySelector('#rate-input');
    const quantityInput = document.querySelector('#quantity-input');
    const costInput = document.querySelector('#cost-input');

    // 1. Пробуем взять из инпутов (если юзер их заполнил)
    let finalPrice = cleanAndParseFloat(rateInput ? rateInput.value : '');
    let finalQuantity = cleanAndParseFloat(quantityInput ? quantityInput.value : '');
    let finalAmount = cleanAndParseFloat(costInput ? costInput.value : '');

    // 2. FALLBACK: Если в инпутах пусто, берем со страницы
    if (!finalPrice) {
        console.log('P2P Analytics: Price missing in input, parsing from page...');
        finalPrice = cleanAndParseFloat(parsePriceFromPage());
    }
    if (!finalQuantity) {
        console.log('P2P Analytics: Quantity missing in input, parsing from page...');
        finalQuantity = cleanAndParseFloat(parseQuantityFromPage());
    }
    if (!finalAmount) {
        console.log('P2P Analytics: Amount missing in input, parsing from page...');
        finalAmount = cleanAndParseFloat(parseAmountFromPage());
    }

    // 3. Сохраняем в объект (эти поля уйдут в базу всегда)
    formData.price = finalPrice;
    formData.quantity = finalQuantity;
    formData.amount = finalAmount;

    // --- ФОРМИРОВАНИЕ ЧЕКА (ЕСЛИ НУЖЕН) ---
    if (formData.hasReceipt) {
        console.log('P2P Analytics: Collecting receipt data...');
        
        // Округляем количество для чека (требование Эвотора)
...
async function injectWidgetUnderChat() {
    ...
    // 2. Ищем поле ввода, чтобы от него оттолкнуться
    const chatInputBlock = document.querySelector('.im-container__bottom');
    if (!chatInputBlock) return;

    // 3. !!! КЛЮЧЕВОЙ МОМЕНТ !!!
    // Нам нужно найти ГЛАВНЫЙ контейнер чата. 
    // Обычно в Bybit структура такая: .im-container -> .im-container__main / .im-container__bottom
    // Мы ищем ближайшего родителя с классом .im-container или .chat-box (зависит от версии верстки)
    // Используем closest, чтобы подняться вверх по дереву.
    const mainChatWrapper = chatInputBlock.closest('.im-container') || chatInputBlock.parentElement.parentElement;

    if (!mainChatWrapper) return;
    ...
    // 5. Проверяем, стоит ли он уже ПОСЛЕ главного контейнера чата
    if (mainChatWrapper.nextElementSibling === widget && widget.classList.contains('embedded-mode')) {
        return; // Всё отлично, он на месте
    }

    console.log('P2P Analytics: Выносим виджет из чата и ставим ПОД ним...');

    // 6. Настраиваем стили
    widget.classList.add('embedded-mode');
    widget.classList.remove('collapsed');
    
    // 7. ВСТАВЛЯЕМ ПОСЛЕ ГЛАВНОГО КОНТЕЙНЕРА ЧАТА
    // Это гарантирует, что виджет будет отдельным блоком, а не частью чата
    mainChatWrapper.insertAdjacentElement('afterend', widget);
}
// --- Main Execution ---
