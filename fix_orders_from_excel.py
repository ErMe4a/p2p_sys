...
Запуск рядом с manage.py:
    ...
    python fix_orders_from_excel.py bybit_diff_2026-03-26.xlsx

    # Применить изменения:
    python fix_orders_from_excel.py bybit_diff_2026-03-26.xlsx --update

Читает Excel с расхождениями и обновляет только те ордера которые в нём указаны.
Берёт значения из колонки "На Bybit" (правильные данные с биржи).
"""
...
def main():
    ...
    if not xlsx_path:
        print("Укажи файл: python fix_orders_from_excel.py bybit_diff_2026-03-26.xlsx")
        sys.exit(1)
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        new_value = row[4]  # колонка "На Bybit"
