...
async function createUnifiedFormSection() {
    ...
    // Нормализуем результат, чтобы не падать на undefined/null
    const ok = !!(bankDetailsResult && bankDetailsResult.success);
    const bankList = Array.isArray(bankDetailsResult?.data) ? bankDetailsResult.data : [];

    // Вспомогательная функция: перерисовать меню банков
    const renderBankMenu = (banks) => {
        dropdownMenu.innerHTML = '';

        banks.forEach(bankDetail => {
    ...
    if (!ok) {
        ...
        // НЕ ошибка: просто пока нет реквизитов
        buttonTextSpan.textContent = 'Банк не добавлен';

        dropdownMenu.innerHTML = '';

        const infoItem = document.createElement('div');
        infoItem.className = 'p2p-analytics-menu-item';
        infoItem.textContent = 'Список пуст. Добавьте реквизит в панели/админке Django.';
        infoItem.style.cursor = 'default';
        infoItem.style.opacity = '0.85';
        dropdownMenu.appendChild(infoItem);

        const refreshItem = document.createElement('div');
        refreshItem.className = 'p2p-analytics-menu-item';
        refreshItem.textContent = '↻ Обновить список';
        ...
        refreshItem.onclick = async (e) => {
            ...
            try {
                const r = await fetchBankDetails();
                ...
                if (r?.success) {
                    if (banks.length === 0) {
                        ...
                        info.className = 'p2p-analytics-menu-item';
                        info.textContent = 'Список всё ещё пуст. Добавьте реквизит и нажмите обновить.';
                        info.style.cursor = 'default';
                        info.style.opacity = '0.85';
                        dropdownMenu.appendChild(info);

                        // снова кнопка обновления
                        dropdownMenu.appendChild(refreshItem);
                    } else {
                        buttonTextSpan.textContent = 'Выберите банк';
                        renderBankMenu(banks);
                    }
                } else {
                    buttonTextSpan.textContent = 'Ошибка загрузки банков';
                    const err = document.createElement('div');
                    err.className = 'p2p-analytics-menu-item';
                    err.textContent = 'Не удалось обновить список банков';
                ...
                errItem.className = 'p2p-analytics-menu-item';
                errItem.textContent = 'Ошибка при обновлении списка';
                errItem.style.color = '#ff6b6b';
                errItem.style.cursor = 'default';
                dropdownMenu.appendChild(errItem);
                console.error('P2P Analytics: Error refreshing bank details:', err);
            }
        };
        dropdownMenu.appendChild(refreshItem);

    } else {
        // success=true и банки есть
        buttonTextSpan.textContent = 'Выберите банк';
        console.log('P2P Analytics: Bank details loaded successfully, count:', bankList.length);
        renderBankMenu(bankList);
    }
        ...
        checkOrderExists(orderId).then(orderResult => {
            if (orderResult.success && orderResult.exists && orderResult.data) {
                ...
                if (order.details) {
                    ...
                    const matchingBankDetail = bankList.find(bd => bd.id === details.id);
...
function findBybitChatHeaderContainer() {
  // 1) Самый надёжный якорь на Bybit: блок, где есть "Report User" и имя контрагента
  const all = Array.from(document.querySelectorAll("div,section,header"));

  // Ищем контейнер, который содержит "Report" (кнопка репорта) и выглядит как шапка карточки пользователя
  const reportNode = all.find(el => (el.innerText || "").includes("Report User") || (el.innerText || "").includes("Report"));
  if (reportNode) {
    // Поднимаемся наверх до "карточки" (обычно 3-6 уровней)
    let cur = reportNode;
    for (let i = 0; i < 8 && cur; i++) {
      // эвристика: шапка обычно не огромная и не вся страница
      const rect = cur.getBoundingClientRect();
      if (rect.height > 60 && rect.height < 220 && rect.width > 250) {
        return cur;
      }
      cur = cur.parentElement;
    }
    return reportNode.parentElement || reportNode;
  }

  // 2) Фоллбек: ищем элемент с именем контрагента (Maxim MSK) и поднимаемся вверх
  const nameEl = Array.from(document.querySelectorAll("h1,h2,h3,div,span,a"))
    .find(el => (el.textContent || "").trim() === "Maxim MSK");
