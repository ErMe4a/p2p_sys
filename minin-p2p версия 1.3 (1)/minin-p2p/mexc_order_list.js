...
(function(){
    ...
    function formatBadgeState(order) {
        ...
        // ИСПРАВЛЕНИЕ:
        // Проверяем наличие uuid. Если uuid null, значит чека нет.
    ...
    async function processRow(row, version) {
        ...
        return requestQueue.add(async () => {
            ...
            try {
                ...
                // ── Ищем элемент со статусом ────────────────────────────────
                // Новый дизайн (ANT table): div.page_statusColText__md_kg внутри td
                // Старый дизайн: div[class*="tabListSix"] или div[class*="status"]
                let statusTextEl =
                    row.querySelector('.page_statusColText__md_kg, [class*="statusColText"]') ||
                    row.querySelector('.page_tabListSix__s04Ms, [class*="tabListSix"]') ||
                    row.querySelector('[class*="status"], [class*="Status"]');

                // Проверяем что ордер завершён — если нет, бейджи не нужны
                const statusText = statusTextEl ? (statusTextEl.textContent || '') : '';
                const isCompleted = /Завершен|Completed|завершен|completed/i.test(statusText);
                if (!isCompleted) {
                    console.log('[P2P MEXC] Order not completed, skipping badges for:', orderId, 'status:', statusText.trim());
                    row.setAttribute('data-p2p-status', 'done');
                    return;
                }

                // ── Выбираем контейнер для вставки бейджей ───────────────────
                // Новый дизайн: вставляем в div внутри td статуса (родитель statusTextEl)
                // Старый дизайн: вставляем в сам statusContainer
                let targetContainer = null;

                if (statusTextEl) {
                    // Поднимаемся до ближайшего div-контейнера (не td, не tr)
                    let parent = statusTextEl.parentElement;
                    if (parent && parent.tagName === 'TD') {
                        // statusTextEl сразу в td — оборачиваем в div или вставляем перед ним
                        targetContainer = statusTextEl.parentElement; // сам td
                    } else if (parent) {
                        targetContainer = parent; // div внутри td
                    } else {
                        targetContainer = statusTextEl;
                    }
                }

                if (!targetContainer) {
                    // Fallback: первая ячейка строки
                    targetContainer = row.querySelector('.page_orderTabInfo__6ggOE, [class*="orderTabInfo"]') 
    ...
    async function processAllVisibleRows(version) {
        // MEXC fiat-order-list page structure (new ANT Design table):
        // Each order is in tr.ant-table-row[data-row-key]
        
        // Method 1: ANT Design table rows (new design)
        let rows = $all('tr.ant-table-row[data-row-key]');
        
        // Method 2: Old MEXC card layout (legacy)
        if (rows.length === 0) {
            rows = $all('.page_orderTableContent__kF0J0, [class*="orderTableContent"]');
        }
            ...
            for (const mutation of mutations) {
                ...
                for (const node of mutation.addedNodes) {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        // Новый дизайн: tr.ant-table-row или tbody с такими строками
                        // Старый дизайн: div[class*="orderTableContent"]
                        if (node.matches && (
                            ...
                            (node.querySelector && (
                                node.querySelector('tr.ant-table-row[data-row-key]') ||
                                node.querySelector('[class*="orderTableContent"]')
                            ))
                        )) {
