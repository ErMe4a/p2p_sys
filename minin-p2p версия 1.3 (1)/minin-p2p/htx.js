        ...
        try {
            ...
            try {
                ...
                if (existingOrderResult.success && existingOrderResult.exists && existingOrderResult.data) {
                    ...
                    console.log('P2P Analytics HTX: Existing order found:', existingOrderData);
                }
            } catch (error) {
                console.warn('P2P Analytics HTX: Could not check existing order:', error);
            }

            // Step 3: Определяем логику чека
            // КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ:
            // Если в API вернулся receipt - значит в нем есть uuid и чек уже пробит
            // Если receipt не вернулся - либо чека нет, либо чек без uuid (не пробит)
            let finalReceipt = null;
            
            if (formData.hasReceipt) {
                // Пользователь хочет пробить чек сейчас
                finalReceipt = formData.receipt;
                console.log('P2P Analytics HTX: Using NEW receipt for order');
            } else if (existingOrderData && existingOrderData.receipt) {
                // Если receipt есть в ответе API - значит чек уже пробит (есть uuid)
                // Сохраняем существующий чек
                finalReceipt = existingOrderData.receipt;
                console.log('P2P Analytics HTX: Keeping existing receipt with uuid');
            }
            // Если receipt нет в API - оставляем null

            // Step 4: Prepare and save order data
            ...
            console.log('P2P Analytics HTX: Sending Payload:', orderData);

            const result = await saveOrder(orderData);

            if (result.success) {
                // Step 5: Upload screenshot if available
...
function parseOrderInfo() {
    ...
    // Регулярка для даты: 2024-01-30 15:30:00
    const dateRegex = /(\d{4}[-/]\d{2}[-/]\d{2}\s+\d{2}:\d{2}:\d{2})/;

    try {
        console.log('P2P Analytics HTX: Searching for creation date...');

        // === ПРИОРИТЕТ 1: Ищем строго "Последняя сделка со мной" в .user-list ===
        ...
        if (userListItems.length > 0) {
            for (const li of userListItems) {
                ...
                // ВАЖНО: Проверяем наличие ключевой фразы ПЕРЕД поиском даты.
                // Строку "Последний вывод" мы игнорируем, так как в ней нет фразы "сделка".
                if (text.includes('Последняя сделка') || text.includes('Latest deal')) {
                    
                    const match = text.match(dateRegex);
                    if (match) {
                        // Меняем слеши на тире (на всякий случай)
                        ...
                        if (!isNaN(parsedDate.getTime())) {
                            ...
                            // Извлекаем "чистую" строку даты для логов (как в HTML: 2025-01-18 21:30:28)
                            // Но сохраняем в ISO формате для базы данных
                            console.log('P2P Analytics HTX: ✓ FOUND TARGET DATE (Last Deal):', dateStr);
                            dateFound = true;
                            break; // СТОП! Мы нашли то, что искали. Дальше не идем.
                        }
                    }
                }
            }
        ...
        // === ПРИОРИТЕТ 2: Если в user-list ничего не нашли, ищем в деталях ордера ===
        // Это запасной вариант на случай, если блок user-list отсутствует или там нет сделок
        if (!dateFound) {
            ...
            for (const block of contentBlocks) {
                ...
                if (match) {
                    ...
                    if (!isNaN(parsedDate.getTime())) {
                        orderInfo.createdAt = parsedDate.toISOString();
                        console.log('P2P Analytics HTX: ✓ Found date in trade detail block:', dateStr);
                        dateFound = true;
                        break;
                    }
                }
        ...
        // === ПРИОРИТЕТ 3: Ищем по всему тексту страницы (на крайний случай) ===
        if (!dateFound) {
            ...
            if (globalMatch) {
                ...
                if (!isNaN(parsedDate.getTime())) {
                    orderInfo.createdAt = parsedDate.toISOString();
                    console.log('P2P Analytics HTX: ✓ Found date in BODY text:', dateStr);
                    dateFound = true;
                }
            }
        }
    ...
    // Если вообще ничего не нашли — ставим текущее время
    if (!dateFound) {
        console.warn('P2P Analytics HTX: ⚠ Date not found anywhere, using CURRENT time.');
        orderInfo.createdAt = new Date().toISOString();
    }
    
    // Определяем тип (BUY/SELL)
        ...
        for (const item of amountElements) {
            ...
            if (label && value) {
                ...
                if (/USDT|BTC|ETH|USDC/i.test(valueText)) {
                    console.log('P2P Analytics HTX: Found quantity text:', valueText);
                    
                    // Удаляем всё кроме цифр и точки. 
                    // Заменяем запятую на точку, если она используется как разделитель дроби
                    // HTX пример: "1,234.56 USDT" -> "1234.56"
                    let cleanValue = valueText.replace(/[^\d.,]/g, '');
                    
                    // Если есть и точка и запятая, значит запятая - разделитель тысяч (удаляем её)
...
function createCheckContent() {
    ...
    // Контакт (Email/Phone) - скрыт по умолчанию
    ...
    (async () => {
        ...
        Promise.all([orderCheckPromise, credentialsCheckPromise]).then(([orderResult, hasCredentials]) => {
            console.log('P2P Analytics HTX: Order check result:', orderResult);
            
            if (orderResult.success && orderResult.exists && orderResult.data) {
                const order = orderResult.data;
                
                // КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ:
                // В API receipt возвращается только если есть uuid
                // Если receipt есть в ответе - значит чек уже пробит и есть uuid
                // Если receipt нет в ответе - либо чека нет, либо чек без uuid (не пробит)
                
                if (order.receipt) {
                    // ЧЕК С UUID - уже пробит
                    receiptExists = true;
                    ...
                    // ЧЕКА НЕТ ИЛИ ЧЕК БЕЗ UUID (не пробит)
                    // Заполняем цифры из order (они теперь всегда приходят из API)
                    ...
                    // Не блокируем форму, можно редактировать
                }
                
                // Проверяем учетные данные Эвотора
                if (!hasCredentials) {
                    checkbox.disabled = true;
    ...
    checkbox.addEventListener('change', () => {
        if (!receiptExists) {
            // Чекбокс управляет только видимостью контакта
            ...
            if (checkbox.checked) {
                ...
                // При включении еще раз пробуем подтянуть данные, если пусто
...
// Вспомогательная функция: жестокая очистка чисел
function strictParseFloat(value) {
    if (!value) return null;
    if (typeof value === 'number') return value;
    
    // 1. Превращаем в строку и удаляем ВСЕ пробелы (включая неразрывные \u00A0)
    let clean = value.toString().replace(/\s|&nbsp;/g, '').trim();
    
    // 2. Удаляем буквы и валюты (оставляем только цифры, точки, запятые)
    clean = clean.replace(/[^\d.,]/g, '');
    
    if (!clean) return null;

    // 3. Логика разделителей:
    // Если есть и точка и запятая (1,234.56) -> удаляем запятые
    if (clean.includes(',') && clean.includes('.')) {
        clean = clean.replace(/,/g, '');
    } 
    // Если только запятая (1234,56) -> меняем на точку
    else if (clean.includes(',')) {
        clean = clean.replace(',', '.');
    }
...
// Function to collect form data
// Function to collect form data
// Функция сбора данных с "умным" парсингом чисел
function collectFormData() {
    ...
    // Пытаемся взять значения из инпутов
    ...
    console.log('P2P Analytics HTX [Debug Input]:', { 
        rateInput: rateInput?.value, 
        qtyInput: quantityInput?.value, 
        costInput: costInput?.value 
    });

    // --- FALLBACK (ЗАПАСНОЙ ПЛАН) ---
    // Если в инпутах пусто (или 0), принудительно парсим страницу
    if (!finalPrice) {
        console.log('P2P Analytics HTX: Price missing, parsing from page...');
        finalPrice = strictParseFloat(parsePriceFromPage());
    }
    if (!finalQuantity) {
        console.log('P2P Analytics HTX: Quantity missing, parsing from page...');
        finalQuantity = strictParseFloat(parseQuantityFromPage());
    }
    if (!finalAmount) {
        console.log('P2P Analytics HTX: Amount missing, parsing from page...');
        finalAmount = strictParseFloat(parseAmountFromPage());
    }

    console.log('P2P Analytics HTX [Debug Final]:', { finalPrice, finalQuantity, finalAmount });

    // --- 4. Сохранение в объект ---
    // Эти поля уходят в базу ВСЕГДА (независимо от галки)
    formData.quantity = finalQuantity; // Может быть null, если совсем ничего не нашли
    formData.price = finalPrice;
    formData.amount = finalAmount;
    
    // --- 5. Обработка даты ---
    // Берем распаршенную дату
    const orderInfo = parseOrderInfo();
    formData.createdAt = orderInfo.createdAt;
    formData.type = orderInfo.type;

    // --- 6. Данные для чека (только если галка стоит) ---
    if (formData.hasReceipt) {
        // Округляем до 3 знаков для Эвотора
