...
# Валюты, по которым чеки пока НЕ пробиваются вообще (сейчас закрыт ETH; BTC
# открыт — у него количество пробивается ×100000, см. evotor_atol.QUANTITY_SCALE).
# Ордер при этом сохраняется как обычно (оборот и
...
RECEIPT_BLOCKED_CURRENCIES = ("ETH",)
RECEIPT_BLOCKED_MESSAGE = (
    f"Чеки по {' и '.join(RECEIPT_BLOCKED_CURRENCIES)} пока не пробиваются: "
    "ордер сохранён, но чек по нему не создан."
)
