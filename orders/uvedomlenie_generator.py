...
# Ставки УСН - ЛЬГОТНЫЕ РЕГИОНАЛЬНЫЕ (ст. 346.20 НК РФ), а не федеральные
# 6%/15%. Подтверждено: у ИП в этой системе региональная пониженная
# ставка. Совпадает со ставкой, которую использует вкладка "Прибыль"
# (_calc_ndfl в views.py) - если ставка когда-либо изменится регионом,
# менять нужно СРАЗУ в обоих местах (иначе снова разъедутся суммы).
USN_INCOME_RATE = Decimal("0.01")
USN_INCOME_OUTCOME_RATE = Decimal("0.15")  # для "доходы-расходы" ставка не уточнялась, оставлена федеральная
