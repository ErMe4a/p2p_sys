...
def _get_mexc_side_via_pagination(user, order_id: str, order_date: datetime = None) -> str | None:
    ...
    try:
        ...
        page = 1
        # ИСПРАВЛЕНО: у активных аккаунтов (сотни ордеров/день) нужный ордер
        # может оказаться дальше 10-й страницы даже внутри правильного
        # окна дат (подтверждено на реальном случае — нашёлся только на 4-й
        # из потенциальных ~200+ страниц). Подняли до 40 (реально означает
        # до 2000 ордеров в окне) — компромисс между покрытием и временем ответа.
        while page <= 40:
            timestamp = int(time.time() * 1000)
            params = {
                "page": page,
