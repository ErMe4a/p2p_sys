...
def build_receipt_payload_v5(order, user, receipt_data: dict, check_type: str) -> dict:
    ...
    # 6. ИСПРАВЛЕНО: раньше здесь для BTC стояла точность 8 знаков (чтобы не
    # обнулять маленькие суммы) — но у Evotor поле quantity жёстко принимает
    # МАКСИМУМ 3 знака после запятой для любой валюты, при 4+ весь запрос
    ...
    # Защита от нуля ПОСЛЕ усечения — маленькие BTC-суммы (<0.001 BTC) могут
    # обнулиться при округлении до 3 знаков; 0 Evotor тоже не принимает и
    # деление на 0 ниже упадёт с ошибкой. Подставляем минимально возможное
    # ненулевое значение при таком лимите точности.
    if quantity <= 0:
        quantity = 0.001
