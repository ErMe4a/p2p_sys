...
def get_required_fns_documents(user, year, for_user_id=None):
    ...
    # НДС показываем только ОСНО. УСН и Патент НДС не платят в принципе -
    # build_nds_declaration для них всегда кидает ValueError с текстом
    # "формируется только для режима ОСНО", а этот текст не совпадает ни с
    # одним _FNS_SKIP_MARKERS (там "не формируется", тут "формируется
    # только для") - раньше это приводило к тому, что декларация НДС
    # попадала в список документов пользователя как ok=False/ошибка, и
    # is_fns_all_submitted навсегда держал статус 'pending' у любого
    # активного УСН-щика, хотя сдавать ему нечего.
    show_nds_block = is_osno
