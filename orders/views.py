...
def admin_login(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ИСПРАВЛЕНО: раньше ошибки формы (AuthenticationForm — англоязычный
            # текст, LANGUAGE_CODE='en-us') нигде не показывались — шаблон
            # custom_admin/login.html не выводит ни form.errors, ни messages,
            # так что при неверном пароле форма просто перезагружалась пустой,
            # без единого объяснения (тот же класс проблемы, что раньше нашли
            # на обычной странице входа трейдеров, history.md §46.3). Показываем
            # свой понятный текст вместо непереведённой ошибки формы.
