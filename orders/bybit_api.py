...
def sync_bybit_orders(user):
    ...
    # ── 7. Самоисправление гонки ──────────────────────────────────────────────
    # processed_ids (шаг 3) — это снимок НА МОМЕНТ НАЧАЛА синка. Пока мы тут
    ...
    stale_ids = (
        ...
        & set(UnprocessedOrder.objects.filter(user=user).values_list("order_id", flat=True))
    )
    if stale_ids:
        deleted, _ = UnprocessedOrder.objects.filter(user=user, order_id__in=stale_ids).delete()
        logger.info("Bybit [%s]: подчищено %d гонок в Необработанных.", user.username, deleted)
